...
type CampaignMeta = {
  ...
  productFilter?: { tag?: string; limit?: number };
};
...
export default function CampaignMerchandisingBoard({ slug: slugProp }: { slug: string }) {
  const slug = parseCampaignSlug(slugProp);
  const { user } = useAuth();
  ...
  const [mode, setMode] = useState<CampaignMerchandisingMode>('auto');
  ...
  const setModeLive = useCallback((next: CampaignMerchandisingMode) => {
  ...
  const loadMerchandising = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/campaigns/${slug}/merchandising`, { headers });
      if (!res.ok) {
  ...
  const fetchPool = useCallback(async (excludeKeys?: string[]) => {
    if (!slug) return;
    const currentUser = userRef.current;
    if (!currentUser || modeRef.current === 'auto') return;

    const showSpinner = poolItemsRef.current.length === 0;
    ...
    try {
      ...
      if (q) params.set('q', q);
      const res = await fetch(`/api/admin/campaigns/${slug}/merchandising/pool?${params}`, {
        headers,
      });
      if (!res.ok) return;
      const data = await res.json();
  ...
  useEffect(() => {
    if (slug && user?.uid) loadMerchandising();
  }, [loadMerchandising, user?.uid, slug]);

  // Only refetch pool when search/mode changes — not when curated order or keys change.
  useEffect(() => {
    if (!slug || mode === 'auto' || !user?.uid) return;
    let cancelled = false;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/campaigns/${slug}/merchandising`, {
        method: 'PUT',
        headers,
  ...
  const handleSync = async (strategy: 'replace' | 'append', save: boolean) => {
    if (!slug || !user) return;
    const modeAtSyncStart = modeRef.current;
    setSyncing(true);
    setError(null);
    try {
      const headers = await getAdminAuthHeaders(user);
      const res = await fetch(`/api/admin/campaigns/${slug}/merchandising/sync`, {
        method: 'POST',
        headers,
      ...
      const data = await res.json();
      const nextMode = (
        save ? data.merchandising.mode : modeAtSyncStart
      ) as CampaignMerchandisingMode;
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-16 flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          <h1 className="text-lg font-semibold text-gray-900">Invalid campaign</h1>
          <p className="mt-2 text-sm text-gray-600">Cannot load merchandising without a valid campaign slug.</p>
          <Link href="/admin/campaigns" className="mt-4 inline-block text-sm text-indigo-600 hover:text-indigo-800">
            ← Back to campaigns
          </Link>
        </div>
      </div>
  ...
  const title = campaign?.title?.en || campaign?.title?.he || slug;
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Tag: <span className="font-mono">{campaign?.productFilter?.tag || '—'}</span>
              {' · '}
        ...
        <div className="bg-white shadow rounded-lg p-4 mb-6">
          <div className="flex flex-wrap items-end gap-4">
            <div>
              ...
              <select
                value={mode}
                onChange={(e) => setModeLive(e.target.value as CampaignMerchandisingMode)}
                className="px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-800"
              >
                <option value="auto">Auto (tag order only)</option>
          ...
          <p className="mt-3 text-xs text-gray-500">
            <span className="font-medium text-gray-700">Sync from tag</span> rebuilds your curated
            list from all products with this campaign&apos;s tag (current catalog order).{' '}
            <span className="font-medium text-gray-700">Append new from tag</span> keeps your
            existing order and adds only tag-matched variants that are not already in the list.
            Neither saves until you click Save or Sync &amp; save. Storefront order applies when
            sort is Relevance; price and newest override merchandising.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {mode !== 'auto' && (
            <div className="lg:col-span-1 bg-white shadow rounded-lg p-4 h-fit max-h-[70vh] flex flex-col">
              <h2 className="text-sm font-semibold text-gray-900 mb-3">Add from tag pool</h2>
                  ...
                  poolItems.map((row) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {row.isStale && (
                          <p className="text-[10px] text-amber-700">Incomplete catalog data</p>
                        )}
                      </div>
          ...
          >
            ...
            {mode === 'auto' ? (
              <p className="text-sm text-gray-500 py-8 text-center">
                Switch to Pinned or Manual to arrange products. Use Sync from tag to import the
                current catalog order as a starting point.
              </p>
            ) : rows.length === 0 ? (
              <p className="text-sm text-gray-500 py-8 text-center">
                No variants yet. Sync from tag or add from the pool.
              </p>
            ) : (
              <DndContext
                sensors={sensors}
