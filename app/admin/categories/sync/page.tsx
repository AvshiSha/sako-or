  ...
  const syncCategories = async () => {
    try {
      ...
      if (data.success) {
        ...
        setErrors([data.error || 'Failed to sync categories'])
      }
    } catch (error) {
      console.error('Error syncing categories:', error)
      setErrors(['Failed to sync categories'])
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            <div className="flex items-center">
              ...
              <h1 className="text-3xl font-bold text-gray-900">Sync Categories</h1>
            </div>
            <p className="text-gray-600 mt-2">Synchronize categories between Firebase and Neon DB</p>
          </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <ul className="list-disc list-inside text-blue-700 space-y-1 mb-4">
                  <li><strong>Firebase:</strong> Primary database for categories (used by the website)</li>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Follow these steps to synchronize your categories:
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Compare categories between Firebase and Neon DB</p>
                </div>
                <button
              ...
              {comparisonData && (
                <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                  <div>
                    <h3 className="font-medium text-gray-900">Step 2: Sync Categories</h3>
                    <p className="text-sm text-gray-600">
                      {comparisonData.differences.needsSync 
                        ? `Found ${comparisonData.differences.missingInNeon.length} categories to sync`
                        : 'Categories are already in sync'
                      }
                    </p>
                  </div>
                  <button
                    onClick={syncCategories}
                    disabled={loading || !comparisonData.differences.needsSync}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mb-2">
                        {comparisonData.firebase.count} categories
                      </p>
                      <div className="max-h-32 overflow-y-auto">
                        {comparisonData.firebase.categories.map((category, index) => (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mb-2">
                        {comparisonData.neon.count} categories
                      </p>
                      <div className="max-h-32 overflow-y-auto">
                        {comparisonData.neon.categories.map((category, index) => (
                          <div key={index} className="text-sm text-gray-700 py-1">
                            • {category}
                          </div>
                        ))}
                  ...
                  {comparisonData.differences.missingInNeon.length > 0 && (
                    <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                      ...
                      <div className="text-sm text-yellow-700">
                        {comparisonData.differences.missingInNeon.map((category, index) => (
                  ...
                  {comparisonData.differences.extraInNeon.length > 0 && (
                    <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                      ...
                      <div className="text-sm text-orange-700">
                        {comparisonData.differences.extraInNeon.map((category, index) => (
                          <div key={index}>• {category}</div>
                        ))}
                      </div>
                    </div>
