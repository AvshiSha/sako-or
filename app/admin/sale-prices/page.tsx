...
function SalePriceManagement() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to update sale prices');
      }

      setUpdateResult(data);
      setStep('complete');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to update sale prices');
      setStep('preview');
    } finally {
      setLoading(false);
    }
  ...
  const downloadSkippedRows = () => {
    ...
    const skippedRows = updateResult.details.filter((row) => !row.validation.willUpdate);
    if (skippedRows.length === 0) return;

    const csvLines = ['sku,price,sale_price,reason'];
    skippedRows.forEach((row) => {
      csvLines.push(
        `${row.sku},${row.price},${row.salePrice},"${row.validation.reason || 'Unknown'}"`
      );
    });
    ...
    a.href = url;
    a.download = `skipped-sale-prices-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const validRows = previewData?.filter((r) => r.validation.willUpdate) || [];
  const skippedRows = previewData?.filter((r) => !r.validation.willUpdate) || [];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">End-of-Season Price Update</h1>
                <p className="mt-1 text-sm text-gray-500">
                  Update sale prices in bulk from CSV file
                </p>
              </div>
            </div>
            {step !== 'upload' && (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="ml-3 flex-1">
              ...
              <div className="mt-2 text-sm text-blue-700">
                ...
                <ul className="list-disc list-inside space-y-1 ml-2">
                  <li>
                    <code className="bg-blue-100 px-1 py-0.5 rounded">sku</code>: Product SKU in
                    format xxxx-xxxx (e.g., 4519-0001)
                  </li>
                  <li>
                    <code className="bg-blue-100 px-1 py-0.5 rounded">price</code>: Original (regular) price
                    (e.g., 799 or 799.00)
                  </li>
                  <li>
                    <code className="bg-blue-100 px-1 py-0.5 rounded">sale_price</code> or{' '}
                    <code className="bg-blue-100 px-1 py-0.5 rounded">sale price</code>: Discounted sale price
                    (e.g., 559 or 559.00)
                  </li>
                </ul>
                ...
                <p className="mt-3 text-xs">
                  <strong>Note:</strong> Rows with empty or zero sale prices will be skipped. Sale prices
                  greater than original prices will also be skipped.
                </p>
              </div>
            </div>
          </div>
        ...
        {step === 'preview' && previewData && (
          <div className="space-y-6">
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-4">
                ...
                <div className="bg-green-50 overflow-hidden shadow rounded-lg">
                  <div className="px-4 py-5 sm:p-6">
                    <dt className="text-sm font-medium text-green-700 truncate">Will Update</dt>
                ...
                <div className="bg-yellow-50 overflow-hidden shadow rounded-lg">
                  <div className="px-4 py-5 sm:p-6">
                    <dt className="text-sm font-medium text-yellow-700 truncate">Will Skip</dt>
                ...
                <div className="bg-blue-50 overflow-hidden shadow rounded-lg">
                  <div className="px-4 py-5 sm:p-6">
                    <dt className="text-sm font-medium text-blue-700 truncate">Warnings</dt>
                    <dd className="mt-1 text-3xl font-semibold text-blue-900">
                      {previewData.filter((r) => r.validation.warning).length}
                    </dd>
                  </div>
                </div>
              </div>
                    ...
                    {previewData.slice(0, 50).map((row, index) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {row.validation.currentPrice ?? '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {row.validation.currentSalePrice ?? '-'}
                        </td>
                        <td className="px-6 py-4 text-sm">
                          {row.validation.warning && (
                            <div className="flex items-start">
                              <ExclamationTriangleIcon className="h-4 w-4 text-yellow-500 mr-1 mt-0.5 flex-shrink-0" />
                              <span className="text-yellow-700">{row.validation.warning}</span>
                            </div>
                          )}
                          {!row.validation.willUpdate && row.validation.reason && (
                            <span className="text-yellow-700">{row.validation.reason}</span>
                          )}
                          {row.validation.willUpdate && !row.validation.warning && (
                            <span className="text-green-700">Will update</span>
                          )}
                        </td>
                      </tr>
                    ))}
            ...
            <div className="flex justify-end space-x-4">
              ...
              >
                Apply Updates ({validRows.length} products)
              </button>
            </div>
          </div>
        )}
            ...
            <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
              <div className="mt-3">
                ...
                <h3 className="text-lg font-medium text-gray-900 text-center mt-5">
                  Confirm Sale Price Update
                </h3>
                <div className="mt-2 px-7 py-3">
                  <p className="text-sm text-gray-500 text-center">
                    Are you sure you want to update <strong>{validRows.length}</strong> products with
                    new sale prices?
                  </p>
        ...
        {step === 'updating' && (
          <div className="bg-white shadow rounded-lg p-12">
            <div className="text-center">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">Updating Sale Prices...</h3>
              <p className="mt-2 text-sm text-gray-500">
                Please wait while we update the products in Firestore.
              </p>
            </div>
          </div>
        )}
        ...
        {step === 'complete' && updateResult && (
          <div className="space-y-6">
                ...
                <div className="ml-3 flex-1">
                  ...
                  >
                    {updateResult.success
                      ...
                      : 'Sale Price Update Completed with Warnings'}
                  </h3>
            ...
            {/* Skipped by Reason */}
            {Object.keys(updateResult.skippedByReason).length > 0 && (
              <div className="bg-white shadow rounded-lg overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-yellow-50">
                  <h2 className="text-lg font-medium text-yellow-900">
                    Skipped Rows by Reason ({updateResult.skipped})
                  </h2>
                </div>
                <div className="px-6 py-4">
                  <ul className="space-y-2">
                    {Object.entries(updateResult.skippedByReason).map(([reason, count]) => (
            ...
            {updateResult.warnings.length > 0 && (
              <div className="bg-white shadow rounded-lg overflow-hidden">
                ...
                <div className="px-6 py-4">
                  <ul className="space-y-2">
                    {updateResult.warnings.map((warning, index) => (
